...
//extern unsigned long long int software_hardware_count[1 + NUM_KERNELS];
//extern long long int time_in_line;
//extern int num_reads;

extern int kernel_expanding[NUM_KERNELS];
unsigned long int fifo_queue[NUM_THREADS];
int first = 0;
int last = 0;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	int which_kernel = NUM_KERNELS, aux;
	//clock_t t;
	if ((r_len > 1000) && (q_len > 1000)){
		
		//enter thread queue
		pthread_mutex_lock(&mutex);
		fifo_queue[last] = pthread_self();
		if (last != NUM_THREADS - 1){ last += 1; }
		else{ last = 0; }
		pthread_mutex_unlock(&mutex);
		
		//t = clock();
		while (which_kernel == NUM_KERNELS){
			pthread_mutex_lock(&mutex);
			for (aux = 0; aux < NUM_KERNELS; aux++){
				if ((kernel_expanding[aux] == 0) && (pthread_self() == fifo_queue[first])){
					kernel_expanding[aux] = 1;
					if (first != NUM_THREADS - 1){ first += 1; }
					else{ first = 0; }
					which_kernel = aux;
					break;
				}
		...
		//t = clock() - t;
		//time_in_line += (int)(((float)t)*1000000/CLOCKS_PER_SEC);
	...
	if (which_kernel != NUM_KERNELS){
		
		//software_hardware_count[which_kernel + 1] += 1;
		
		gactx_align(km, r_len, q_len, qlen, mi, rid, rs, rs0, rs1, &re, re0, &re1, qseq0, rev, qs, &qe, qe0, &qe1, r, which_kernel);
